...
import com.example.demo.model.SocketConfig;
import com.example.demo.model.SocketUserInfo;
...
public class SocketController {
    ...
    public void onOpen(Session session, @PathParam(value="role") Integer role) {
        ...
        if (role.equals(0)) {
            ...
            if (findLineUser() != null){
                ...
                //客服显示用户信息
                result.put("msg1", "Being served to user "+userInfo.getSessionId());
                sendMsg(serverInfo.getSession(), JSON.toJSONString(result));
                //告诉用户有客服为他服务
                result.put("msg1", "Customer Service "+serverInfo.getSessionId()+" is at your service！");
        ...
        if (role.equals(1)) {
            ...
            if (findFreeServer() != null){
                ...
                Map<String, String> result = new HashMap<>();
                //客服显示用户信息
                result.put("msg1", "In service to the user "+userInfo.getSessionId());
                sendMsg(serverInfo.getSession(), JSON.toJSONString(result));
                result.put("msg1", "Customer service "+serverInfo.getSessionId()+" is at your service");
                sendMsg(userInfo.getSession(), JSON.toJSONString(result));
            } else {
                //告诉用户系统繁忙
                resultMap.put("msg1", "Queuing, " + userSessionMap.size() +" customers ahead of you...");
            }
    ...
    public void onClose(Session session) {
        ...
        if (serverInfo != null) {
            ...
            if (null != serverInfo.getTargetSessionId()){
                //给用户说系统错误
                Map<String, String> result = new HashMap<>();
                result.put("msg1", "System error, please refresh and try again!");
            ...
            for (SocketUserInfo serverSocketInfo: serverSessionMap.values()) {
                ...
                if (serverSocketInfo.getTargetSessionId() != null && serverSocketInfo.getTargetSessionId().equals(session.getId())){
                    ...
                    if (lineUser != null){
                        ...
                        Map<String, String> result = new HashMap<>();
                        //客服显示用户信息
                        result.put("msg1", "Being served to user "+lineUser);
                        sendMsg(serverSocketInfo.getSession(), JSON.toJSONString(result));
                        //用户显示客户信息
                        result.put("msg1", "Customer service "+serverSocketInfo.getSessionId()+" is at your service");
    ...
    //用户和客户端互相传递消息
    @OnMessage(maxMessageSize = 10485763)
...
//                result.put("msg", message);
                ...
                if (null != serverSessionMap.get(session.getId()).getTargetSessionId()){
                    sendMsg(userSessionMap.get(serverSessionMap.get(session.getId()).getTargetSessionId()).getSession(), jsonObject.toJSONString());
                } else{//如果没有就将消息给自己
                    jsonObject.clear();
                    jsonObject.put("msg1", "Not yet connected to the user");
                    sendMsg(session, jsonObject.toJSONString());
                }
...
//                result.put("msg", message);
                ...
                if (null != userSessionMap.get(session.getId()).getTargetSessionId()){
                    sendMsg(serverSessionMap.get(userSessionMap.get(session.getId()).getTargetSessionId()).getSession(), jsonObject.toJSONString());
                } else{//同上
                    jsonObject.clear();
                    jsonObject.put("msg1", "Customer service has not been connected");
                    sendMsg(session,jsonObject.toJSONString());
                }
            }
